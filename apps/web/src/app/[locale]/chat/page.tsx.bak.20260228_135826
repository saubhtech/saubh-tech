...
interface ChatEvent {
  type: 'new_message' | 'translation_ready' | 'stt_ready';
  event_id: string;
  room_id: string;
  sender?: string;
  content?: any;
...
export default function ChatPage() {
  ...
  useEffect(() => {
    ...
    const callMode = searchParams.get('call');
    if (dmId) {
      const target = rooms.find(r => String(r.conversation_id) === dmId);
      if (target) {
        setActiveRoom(target);
        if (callMode === 'video' || callMode === 'audio') {
          // Auto-start call after a brief delay for room to load
          setTimeout(() => {
            const callType = callMode as 'video' | 'audio';
            (async () => {
              try {
                const res = await fetch('/api/chat/call/token', {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + token },
                  body: JSON.stringify({ room_id: Number(dmId) }),
                });
                ...
                socketRef.current?.emit('call:start', {
                  conversationId: dmId,
                  callerName: user?.fname || 'Someone',
                  callType,
                });
              } catch (err) { console.error('Auto-call failed:', err); }
            })();
          }, 1500);
        }
      }
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: 'Outfit, sans-serif', background: '#080b12', color: '#e0e0e0' }}>
      ...
      <div style={{ width: 320, borderRight: '1px solid #1a1f2e', display: 'flex', flexDirection: 'column', background: '#0d1117' }}>
        <div style={{ padding: '20px 16px 12px', borderBottom: '1px solid #1a1f2e', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{
            ...
            background: onlineStatus === 'connected' ? '#22c55e' : onlineStatus === 'connecting' ? '#f59e0b' : '#ef4444',
          }} title={onlineStatus} />
        </div>
        ...
        <div style={{ display: 'flex', gap: 8, padding: '12px 16px', borderBottom: '1px solid #1a1f2e' }}>
          <input
            style={{ flex: 1, padding: '8px 12px', borderRadius: 8, border: '1px solid #2a2f3e', background: '#161b22', color: '#e0e0e0', fontSize: 13, outline: 'none' }}
            placeholder="User ID for DM..."
            value={dmUserId}
            onChange={e => setDmUserId(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && createDm()}
          />
          <button onClick={createDm} style={{ padding: '8px 14px', borderRadius: 8, border: 'none', background: '#3b82f6', color: '#fff', fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>+</button>
        </div>

        {/* Room list */}
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {rooms.length === 0 && <p style={{ padding: 20, textAlign: 'center', color: '#666', fontSize: 14 }}>No conversations yet</p>}
          {rooms.map(room => {
            ...
            const displayName = room.type === 'DM'
              ...
              : (room.title || 'Group');
            const initial = room.type === 'DM' ? (other?.fname || '?')[0] : '👥';
        ...
        {!activeRoom ? (
          ...
          <>
            ...
            <div style={{
              ...
              borderTop: '1px solid #1a1f2e', background: '#0d1117',
            }}>
              {/* Hidden file input */}
              <input
                ref={fileInputRef}
              ...
              <button
                ...
                style={{
                  ...
                  background: '#161b22', color: '#888', transition: 'all 0.2s',
                }}
                title="Attach file"
              >📎</button>
              ...
              <button
                ...
                style={{
                  width: 42, height: 42, borderRadius: '50%', border: 'none', cursor: 'pointer',
                  display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18,
                  background: isRecording ? '#ef4444' : '#161b22',
                ...
                <>
                  <input
                    style={{
                      ...
                      border: '1px solid #2a2f3e', background: '#161b22',
            ...
            <div style={{ display: 'flex', gap: 16, justifyContent: 'center' }}>
              <button
                ...
                style={{
                  ...
                  width: 64, height: 64, fontSize: 24, cursor: 'pointer',
                }}
              >✕</button>
              <button
                onClick={async () => {
                  incomingAudioRef.current?.pause();
                  setIncomingCall(null);
                  // Find the room and join the call
                  const targetRoom = rooms.find(r => r.conversation_id === incomingCall.conversationId);
                  if (targetRoom) setActiveRoom(targetRoom);
                  try {
                    const res = await fetch('/api/chat/call/token', {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json', Authorization: 'Bearer ' + token },
                      body: JSON.stringify({ room_id: Number(incomingCall.conversationId) }),
                    });
                    if (!res.ok) throw new Error('Failed');
                    const data = await res.json();
                    setCallToken(data.token);
                    setCallLkUrl(data.livekitUrl);
                    setCallType(incomingCall.callType === 'audio' ? 'audio' : 'video');
                    setInCall(true);
      ...
      {inCall && activeRoom && (
        <CallView
          roomId={Number(activeRoom.conversation_id)}
          token={callToken}
          livekitUrl={callLkUrl}
          callType={callType}
          onClose={() => {
            setInCall(false); setCallToken(''); setCallLkUrl('');
            if (activeRoom) socketRef.current?.emit('call:end', { conversationId: activeRoom.conversation_id });
          }}
        />
      )}
      <style>{`
