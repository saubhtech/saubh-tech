...
/** Resilient fetch — timeout + 1 auto-retry for network errors */
async function resilientFetch(
  ...
  try {
    ...
    // One auto-retry for transient network errors
    try {
...
function LoginContent() {
  ...
  const handleRegister = async (e: FormEvent) => {
    ...
    try {
      ...
      setRegErr(err.message || 'Network error. Please try again.');
      setRegStep('idle');
    }
  };
  ...
  const verifyOtp = async (code: string) => {
    ...
    setStep('verifying');
    try {
      const res = await resilientFetch(`${API_BASE}/auth/whatsapp/verify-otp`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
