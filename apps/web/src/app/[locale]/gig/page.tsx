...
interface Offering { offerid: string; userid: string; marketid: number; delivery_mode: string | null; offerings: string | null; doc_url: string | null; audio_url: string | null; video_url: string | null; created_at: string; updated_at: string; }
...
export default function GigPage() {
  ...
  const [offerList, setOfferList] = useState<Offering[]>([]);
  ...
  const loadOfferings = useCallback(() => {
    ...
    fetch(API + '/offerings', { credentials: 'include' })
      .then(r => r.json())
      .then(d => {
        if (Array.isArray(d)) setOfferList(d);
        else if (d && Array.isArray(d.data)) setOfferList(d.data);
        else setOfferList([]);
      })
      .catch(() => setOfferList([]))
  ...
  const startAudioRec = async () => {
    ...
    try {
      ...
      if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
        setError('🎙️ Microphone permission denied. Please allow microphone access in your browser settings.');
      } else if (err.name === 'NotFoundError') {
        setError('🎙️ No microphone found. Please connect a microphone.');
      } else {
        setError('🎙️ Could not start audio recording: ' + (err.message || err));
      }
    }
  };
  ...
  const startVideoRec = async () => {
    ...
    try {
      ...
      if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
        ...
        setError('🎥 Could not start video recording: ' + (err.message || err));
      }
    }
  };
  ...
  const handleOfferingSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      const endpoint = editingOfferId ? API + '/offerings/' + editingOfferId : API + '/offerings';
      const method = editingOfferId ? 'PUT' : 'POST';
      const uploadedUrls = await uploadFiles();
      const payload: any = {
        marketid,
        delivery_mode: deliveryMode,
        offerings: offeringText || undefined,
        doc_url: uploadedUrls.doc_url || (docUrl && !docUrl.startsWith('pending://') ? docUrl : undefined),
        audio_url: uploadedUrls.audio_url || (audioUrl && !audioUrl.startsWith('pending://') ? audioUrl : undefined),
        video_url: uploadedUrls.video_url || (videoUrl && !videoUrl.startsWith('pending://') ? videoUrl : undefined),
      };
      if (!editingOfferId) payload.userid = '0';
      const res = await fetch(endpoint, {
        method,
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
      ...
      setSuccess(true);
      loadOfferings();
      setTimeout(() => { resetOfferingForm(); }, 2000);
    } catch (err: any) { setError(err.message || 'Failed to submit'); }
    setSubmitting(false);
  };

  const handleOfferingDelete = async (id: string) => {
    setDeletingOfferId(id);
    try {
      const res = await fetch(API + '/offerings/' + id, { method: 'DELETE', credentials: 'include' });
      if (!res.ok) throw new Error('Delete failed');
      loadOfferings();
    } catch (err: any) { setError(err.message); }
  ...
  const handleOfferingEdit = (o: Offering) => {
    setEditingOfferId(o.offerid);
    setMarketid(o.marketid);
    setDeliveryMode((o.delivery_mode || '').trim());
    setOfferingText(o.offerings || '');
    setDocUrl(o.doc_url || '');
    setAudioUrl(o.audio_url || '');
  ...
  const resetOfferingForm = () => {
    setSectorid(''); setFieldid(''); setPsType(''); setMarketid(''); setDeliveryMode('');
    setOfferingText(''); setDocUrl(''); setAudioUrl(''); setVideoUrl('');
    setDocName(''); setAudioName(''); setVideoName('');
    setDocFile(null); setAudioFile(null); setVideoFile(null); setAudioBlobData(null); setVideoBlobData(null);
    setSuccess(false); setError(''); setEditingOfferId(null);
    clearAudioRec(); clearVideoRec();
  };

  const fmtDate = (d: string | null) => d ? new Date(d).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' }) : '—';
  const fmtMoney = (v: string | null) => { if (!v) return '—'; const n = Number(v); return isNaN(n) ? '—' : '\u20B9' + n.toLocaleString('en-IN', { minimumFractionDigits: 2 }); };
      ...
      <div className="gr-root">
        ...
        <div className="gr-tabbar">
          <div className="gr-tabbar-inner">
            <button className={'gr-tab' + (activeTab === 'requirements' ? ' gr-tab-active' : '')} onClick={() => { if (activeTab !== 'requirements') { resetForm(); setActiveTab('requirements'); } }}>📋 Requirements</button>
            <button className={'gr-tab' + (activeTab === 'offerings' ? ' gr-tab-active' : '')} onClick={() => { if (activeTab !== 'offerings') { resetOfferingForm(); setActiveTab('offerings'); } }}>🎯 Offerings</button>
        ...
        <div className="gr-content">
          ...
          {success && !editingId ? (
            ...
            <form className="gf-form" onSubmit={handleSubmit}>
              <h2 className="gf-form-title">{editingId ? '✏️ Edit Requirement #' + editingId : 'Create New Requirement'}</h2>
              ...
              <div className="gf-field">
                ...
                <div className="gf-file-row">
                  <button type="button" className="gf-btn gf-btn-outline" onClick={() => docRef.current?.click()}>📁 Browse</button>
                  ...
                  {docName && <span className="gf-file-name">📄 {docName}</span>}
                ...
                <div className="gf-media-box">
                  <div className="gf-file-row">
                    {!audioRecording ? (
                      ...
                      <button type="button" className="gf-btn gf-btn-rec-stop" onClick={stopAudioRec}>
                        <span className="gf-pulse" /> ⏹ Stop &nbsp;{fmtTimer(audioTimer)}
                      </button>
                    )}
                  ...
                  {audioBlobUrl && (
                    <div className="gf-playback">
                      ...
                      <div className="gf-playback-meta">
                        <span className="gf-file-name">🎙️ {audioName} ({fmtTimer(audioTimer)})</span>
                        <button type="button" className="gf-btn gf-btn-sm gf-btn-danger" onClick={clearAudioRec}>✕ Remove</button>
                      </div>
                    </div>
                  )}
                  ...
                  {!audioBlobUrl && audioName && (
                    <div className="gf-playback-meta" style={{ marginTop: 8 }}>
                      <span className="gf-file-name">📁 {audioName}</span>
                      <button type="button" className="gf-btn gf-btn-sm gf-btn-danger" onClick={() => { setAudioName(''); setAudioUrl(''); }}>✕</button>
                    </div>
                  )}
                ...
                <div className="gf-media-box">
                  <div className="gf-file-row">
                    {!videoRecording ? (
                      ...
                      <button type="button" className="gf-btn gf-btn-rec-stop" onClick={stopVideoRec}>
                        <span className="gf-pulse" /> ⏹ Stop &nbsp;{fmtTimer(videoTimer)}
                      </button>
                    )}
                  ...
                  {videoBlobUrl && !videoRecording && (
                    <div className="gf-playback">
                      ...
                      <div className="gf-playback-meta">
                        <span className="gf-file-name">🎥 {videoName} ({fmtTimer(videoTimer)})</span>
                        <button type="button" className="gf-btn gf-btn-sm gf-btn-danger" onClick={clearVideoRec}>✕ Remove</button>
                      </div>
                    </div>
                  )}
                  ...
                  {!videoBlobUrl && !videoRecording && videoName && (
                    <div className="gf-playback-meta" style={{ marginTop: 8 }}>
                      <span className="gf-file-name">📁 {videoName}</span>
                      <button type="button" className="gf-btn gf-btn-sm gf-btn-danger" onClick={() => { setVideoName(''); setVideoUrl(''); }}>✕</button>
                    </div>
                  )}
              ...
              <div className="gf-footer">
                ...
                <button type="submit" className="gf-btn gf-btn-primary gf-btn-lg" disabled={submitting || !marketid || !deliveryMode}>
                  {submitting ? 'Saving...' : editingId ? '✏️ Update Requirement' : '🚀 Submit Requirement'}
                </button>
              </div>
            </form>
          )}
                  ...
                  <tbody>
                    {reqList.map(r => (
                      <tr key={r.requirid}>
                        ...
                        <td>{r.marketid}</td>
                        <td>{r.delivery_mode ? (DM[(r.delivery_mode || '').trim()] || r.delivery_mode) : '—'}</td>
                        <td className="gt-files">
                          {r.doc_url && r.doc_url !== 'undefined' && !r.doc_url.startsWith('pending://') && (
                            <a href={r.doc_url} target="_blank" rel="noopener noreferrer" className="gt-file-link gt-file-doc" title="View Document">📄</a>
                          )}
                          ...
                          {r.video_url && r.video_url !== 'undefined' && !r.video_url.startsWith('pending://') && (
                            <button type="button" className="gt-file-link gt-file-video" title="Play Video" onClick={(e) => {
                              ...
                              wrap.innerHTML = '<div class="gt-video-overlay"></div><div class="gt-video-modal"><video controls autoplay src="' + r.video_url + '"></video><button class="gt-video-close">✕</button></div>';
                          ...
                          {(r.doc_url?.startsWith('pending://') || r.audio_url?.startsWith('pending://') || r.video_url?.startsWith('pending://')) && !r.doc_url?.startsWith('/') && !r.audio_url?.startsWith('/') && !r.video_url?.startsWith('/') && '⏳'}
                        </td>
                        ...
                        <td className="gt-actions">
                          <button className="gf-btn gf-btn-sm gf-btn-outline" onClick={() => handleEdit(r)}>✏️ Edit</button>
                          <button className="gf-btn gf-btn-sm gf-btn-danger" disabled={deletingId === r.requirid} onClick={() => handleDelete(r.requirid)}>
                            {deletingId === r.requirid ? '...' : '🗑️ Del'}
                          </button>
                        </td>
                      </tr>
                    ))}
          ...
          {activeTab === 'offerings' && (<>
          {/* ── Offerings Form ── */}
          {success && !editingOfferId ? (
            <div className="gf-success">
              <div className="gf-success-icon">✓</div>
              <h2>Offering Posted Successfully!</h2>
              <p>Your offering is now visible to clients.</p>
              <button className="gf-btn gf-btn-primary" onClick={resetOfferingForm} style={{ marginTop: 16 }}>Post Another Offering</button>
            </div>
          ) : (
            <form className="gf-form" onSubmit={handleOfferingSubmit}>
              <h2 className="gf-form-title">{editingOfferId ? '✏️ Edit Offering #' + editingOfferId : 'Create New Offering'}</h2>
              {error && <div className="gf-error">{error}</div>}

              {/* Sector */}
              <div className="gf-field">
                <label className="gf-label">Sector <span className="gf-req">*</span></label>
                <select className="gf-select" value={sectorid} onChange={e => setSectorid(e.target.value ? Number(e.target.value) : '')} required={!editingOfferId}>
                  <option value="">Select a sector...</option>
                  {sectors.map(s => <option key={s.sectorid} value={s.sectorid}>{s.sector}</option>)}
                </select>
              </div>
              ...
              <div className="gf-field">
                <label className="gf-label">Field <span className="gf-req">*</span></label>
                <select className="gf-select" value={fieldid} onChange={e => setFieldid(e.target.value ? Number(e.target.value) : '')} disabled={!sectorid} required={!editingOfferId}>
                  <option value="">{sectorid ? 'Select a field...' : 'Select a sector first'}</option>
                  {fields.map(f => <option key={f.fieldid} value={f.fieldid}>{f.field}</option>)}
                </select>
              </div>
              ...
              <div className="gf-field">
                ...
                <div className="gf-radio-row">
                  <label className={'gf-radio-pill' + (psType === '' ? ' active' : '')}>
                    <input type="radio" name="ops" value="" checked={psType === ''} onChange={() => setPsType('')} /> All
                  </label>
                  {psOptions.map(ps => (
                    <label key={ps} className={'gf-radio-pill' + (psType === ps ? ' active' : '')}>
                      <input type="radio" name="ops" value={ps} checked={psType === ps} onChange={() => setPsType(ps)} /> {PS[ps] || ps}
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div className="gf-field">
                <label className="gf-label">Item <span className="gf-req">*</span></label>
                <select className="gf-select" value={marketid} onChange={e => setMarketid(e.target.value ? Number(e.target.value) : '')} disabled={!fieldid && !editingOfferId} required>
                  <option value="">{fieldid ? 'Select an item...' : 'Select a field first'}</option>
                  {items.map(m => <option key={m.marketid} value={m.marketid}>{m.item} ({PS[m.p_s_ps] || m.p_s_ps})</option>)}
                </select>
              </div>
              ...
              <div className="gf-field">
                ...
                <div className="gf-radio-row">
                  {[{ key: 'PH', label: 'Physical' }, { key: 'DG', label: 'Digital' }, { key: 'PD', label: 'Phygital' }].map(opt => (
                    <label key={opt.key} className={'gf-radio-pill' + (deliveryMode === opt.key ? ' active' : '')}>
                      <input type="radio" name="odm" value={opt.key} checked={deliveryMode === opt.key} onChange={() => setDeliveryMode(opt.key)} /> {opt.label}
                    </label>
                  ))}
                </div>
              </div>
              ...
              {/* Offerings */}
              <div className="gf-field">
                <label className="gf-label">Offerings</label>
                <textarea className="gf-textarea" rows={4} placeholder="Describe what you offer in detail..." value={offeringText} onChange={e => setOfferingText(e.target.value)} />
              </div>
              ...
              <div className="gf-field">
                <label className="gf-label">Documents</label>
                <div className="gf-file-row">
                  <input type="file" ref={docRef} accept="image/*,.pdf,.doc,.docx,.txt" className="gf-file-hidden" onChange={e => { const f = e.target.files?.[0]; if (f) { setDocFile(f); setDocName(f.name); setDocUrl('pending://' + f.name); } }} />
                  <button type="button" className="gf-btn gf-btn-outline gf-btn-sm" onClick={() => docRef.current?.click()}>📁 Browse</button>
                  {docName && <span className="gf-file-name">{docName} <button type="button" className="gf-x" onClick={() => { setDocFile(null); setDocName(''); setDocUrl(''); }}>✕</button></span>}
                  {!docName && docUrl && !docUrl.startsWith('pending://') && <span className="gf-file-name">{docUrl}</span>}
                </div>
              </div>
              ...
              <div className="gf-field">
                <label className="gf-label">Audio</label>
                <div className="gf-file-row">
                  <input type="file" ref={audioFileRef} accept="audio/*" className="gf-file-hidden" onChange={e => { const f = e.target.files?.[0]; if (f) { setAudioFile(f); setAudioName(f.name); setAudioUrl('pending://' + f.name); clearAudioRec(); } }} />
                  <button type="button" className="gf-btn gf-btn-outline gf-btn-sm" onClick={() => audioFileRef.current?.click()}>📁 Browse</button>
                  {!audioRecording && !audioBlobUrl && (
                    <button type="button" className="gf-btn gf-btn-sm" style={{ background: '#10b981', color: '#fff' }} onClick={startAudioRec}>🎙️ Record</button>
                  )}
                  {audioRecording && (
                    <button type="button" className="gf-btn gf-btn-sm gf-btn-danger" onClick={stopAudioRec}>⏹ Stop ({fmtTimer(audioTimer)})</button>
                  )}
                </div>
                {audioBlobUrl && !audioRecording && (
                  <div className="gf-playback">
                    <audio controls src={audioBlobUrl} style={{ width: '100%', marginTop: 8 }} />
                    <div className="gf-playback-meta">
                      <span className="gf-file-name">🎙️ {audioName} ({fmtTimer(audioTimer)})</span>
                      <button type="button" className="gf-btn gf-btn-sm gf-btn-danger" onClick={clearAudioRec}>✕ Remove</button>
                    </div>
                ...
                {!audioBlobUrl && !audioRecording && audioName && (
                  <div className="gf-playback-meta" style={{ marginTop: 8 }}>
                    <span className="gf-file-name">📁 {audioName}</span>
                    <button type="button" className="gf-btn gf-btn-sm gf-btn-danger" onClick={() => { setAudioFile(null); setAudioName(''); setAudioUrl(''); }}>✕</button>
                  </div>
                )}
              </div>
              ...
              <div className="gf-field">
                <label className="gf-label">Video</label>
                <div className="gf-file-row">
                  <input type="file" ref={videoFileRef} accept="video/*" className="gf-file-hidden" onChange={e => { const f = e.target.files?.[0]; if (f) { setVideoFile(f); setVideoName(f.name); setVideoUrl('pending://' + f.name); clearVideoRec(); } }} />
                  <button type="button" className="gf-btn gf-btn-outline gf-btn-sm" onClick={() => videoFileRef.current?.click()}>📁 Browse</button>
                  {!videoRecording && !videoBlobUrl && (
                    <button type="button" className="gf-btn gf-btn-sm" style={{ background: '#f59e0b', color: '#0a1628' }} onClick={startVideoRec}>🎥 Record</button>
                  )}
                  {videoRecording && (
                    <button type="button" className="gf-btn gf-btn-sm gf-btn-danger" onClick={stopVideoRec}>⏹ Stop ({fmtTimer(videoTimer)})</button>
                  )}
                </div>
                <div className="gf-video-container" style={{ display: videoRecording ? 'block' : 'none', marginTop: 10 }}>
                  <video ref={videoPreviewRef} className="gf-video-el" autoPlay muted playsInline />
                  {videoRecording && <div className="gf-video-rec-badge"><span className="gf-pulse" /> REC {fmtTimer(videoTimer)}</div>}
                </div>
                {videoBlobUrl && !videoRecording && (
              ...
              <div className="gf-footer">
                {editingOfferId && (
                  <button type="button" className="gf-btn gf-btn-outline" onClick={resetOfferingForm} style={{ marginRight: 12 }}>Cancel Edit</button>
                )}
                <button type="submit" className="gf-btn gf-btn-primary gf-btn-lg" disabled={submitting || !marketid || !deliveryMode}>
                  {submitting ? 'Saving...' : editingOfferId ? '✏️ Update Offering' : '🚀 Submit Offering'}
                </button>
              </div>
            </form>
          )}
          ...
          {/* ── Offerings Table ── */}
          <div className="gt-section">
            <h2 className="gt-title">Your Offerings</h2>
            {loadingOffers ? (
              <p className="gt-empty">Loading...</p>
            ) : offerList.length === 0 ? (
              <p className="gt-empty">No offerings posted yet.</p>
            ) : (
              <div className="gt-table-wrap">
                <table className="gt-table">
                  <thead>
                  ...
                  <tbody>
                    {offerList.map(o => (
                      <tr key={o.offerid}>
                        <td className="gt-id">#{o.offerid}</td>
                        <td>{o.marketid}</td>
                        <td>{o.delivery_mode ? (DM[(o.delivery_mode || '').trim()] || o.delivery_mode) : '—'}</td>
                        <td className="gt-files">
                          {o.doc_url && o.doc_url !== 'undefined' && !o.doc_url.startsWith('pending://') && (
                            <a href={o.doc_url} target="_blank" rel="noopener noreferrer" className="gt-file-link gt-file-doc" title="View Document">📄</a>
                          )}
                          {o.audio_url && o.audio_url !== 'undefined' && !o.audio_url.startsWith('pending://') && (
                            <button type="button" className="gt-file-link gt-file-audio" title="Play Audio" onClick={(e) => {
                              const existing = (e.currentTarget.parentElement as HTMLElement).querySelector('audio');
                              if (existing) { existing.remove(); return; }
                              const audio = document.createElement('audio');
                              audio.src = o.audio_url!; audio.controls = true; audio.autoplay = true;
                              audio.className = 'gt-inline-player';
                              (e.currentTarget.parentElement as HTMLElement).appendChild(audio);
                            }}>🎧</button>
                          )}
                          {o.video_url && o.video_url !== 'undefined' && !o.video_url.startsWith('pending://') && (
                            <button type="button" className="gt-file-link gt-file-video" title="Play Video" onClick={(e) => {
                              const existing = (e.currentTarget.parentElement as HTMLElement).querySelector('.gt-video-popup');
                              if (existing) { existing.remove(); return; }
                              const wrap = document.createElement('div');
                              wrap.className = 'gt-video-popup';
                              wrap.innerHTML = '<div class="gt-video-overlay"></div><div class="gt-video-modal"><video controls autoplay src="' + o.video_url + '"></video><button class="gt-video-close">✕</button></div>';
                              wrap.querySelector('.gt-video-overlay')!.addEventListener('click', () => wrap.remove());
                              wrap.querySelector('.gt-video-close')!.addEventListener('click', () => wrap.remove());
                              document.body.appendChild(wrap);
                            }}>🎬</button>
                        ...
                        <td style={{ maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis' }}>{o.offerings || '—'}</td>
                        <td>{fmtDate(o.created_at)}</td>
                        <td className="gt-actions">
                          <button className="gf-btn gf-btn-sm gf-btn-outline" onClick={() => handleOfferingEdit(o)}>✏️ Edit</button>
                          <button className="gf-btn gf-btn-sm gf-btn-danger" disabled={deletingOfferId === o.offerid} onClick={() => handleOfferingDelete(o.offerid)}>
                            {deletingOfferId === o.offerid ? '...' : '🗑️ Del'}
                          </button>
                        </td>
                      </tr>
                    ))}
