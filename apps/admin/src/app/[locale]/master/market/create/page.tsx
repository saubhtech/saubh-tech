...
export default function CreateMarketPage() {
  ...
  useEffect(() => {
    ...
    ]).then(([s, f]) => { setSectors(s); setAllFields(f); }).catch(() => {});
  }, []);
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(`${API}/master/markets`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify(body) });
  ...
  return (
    <div>
      <h1 style={pageTitle}>Create Market Item</h1>
      <form onSubmit={submit} style={formCard}>
        ...
        <div style={{ display: 'flex', gap: '12px', marginTop: '16px' }}>
          <button type="submit" disabled={saving} style={submitBtn}>{saving ? 'Saving…' : 'Create Market'}</button>
