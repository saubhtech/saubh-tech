...
export default function CreateFieldPage() {
  ...
  useEffect(() => {
    fetch(`${API}/master/sectors?active=true`, { credentials: 'include' })
      .then(r => r.json()).then(setSectors).catch(() => {});
  }, []);
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(`${API}/master/fields`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify(body) });
  ...
  return (
    <div>
      <h1 style={pageTitle}>Create Field</h1>
      <form onSubmit={submit} style={formCard}>
        ...
        <label style={label}>Code (UPPERCASE) *<input style={input} value={form.code} onChange={handle('code')} required /></label>
        ...
        <div style={{ display: 'flex', gap: '12px', marginTop: '16px' }}>
          <button type="submit" disabled={saving} style={submitBtn}>{saving ? 'Saving…' : 'Create Field'}</button>
